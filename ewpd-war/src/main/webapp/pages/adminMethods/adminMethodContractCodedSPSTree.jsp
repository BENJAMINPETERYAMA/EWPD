...
<f:subview id="tree2">
	...
	<h:inputHidden id="scrollTop" value="#{adminmethodContractCodedSPSTreeBackingBean.scrollTop}" />
	<aa:zoneJSF id="treeZone">
		<jcaa:ajaxTree id="serverTree" value="#{adminmethodContractCodedSPSTreeBackingBean.treeDataModel}"
		...
		<f:facet name="Standard-Benefit">
			<h:panelGroup>
				<h:commandLink styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}"
					actionListener="#{adminmethodContractCodedSPSTreeBackingBean.processActionForBenefit}" action ="#{adminMethodSPSValidationBackingBean.loadAMValidation}"
					id="benefitComp"  onmousedown="javascript:getScroll();navigatePageAction(this.id);">
				<h:outputText value="#{node.description}" styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}" />
				</h:commandLink>
			</h:panelGroup>
		</f:facet>
	...
	<script language="javascript" type="text/javascript">
		...
		<h:inputHidden id="hasValErrors"
					value="#{adminmethodContractCodedSPSTreeBackingBean.withChildren}"></h:inputHidden>
</f:subview>
