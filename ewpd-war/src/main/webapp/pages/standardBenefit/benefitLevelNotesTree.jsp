...
<f:subview id="tree2">
	...
	<h:inputHidden id="scrollTop" value="#{benefitLevelNotesTreeBackingBean.scrollTop}" />
	<aa:zoneJSF id="treeZone">
		<jcaa:ajaxTree id="serverTree" value="#{benefitLevelNotesTreeBackingBean.treeDataModel}"
		...
		<f:facet name="root">
			<h:panelGroup>
				<h:commandLink styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}"
					id="benefitLevel"
					actionListener="#{benefitLevelNotesTreeBackingBean.processActionForBenefitLevel}" 
						action="#{benefitLevelNotesAttachmentBackingBean.getMainPage}"
		...
		<f:facet name="Notes">
			<h:panelGroup>
				<h:commandLink styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}"
					actionListener="#{benefitLevelNotesTreeBackingBean.processActionForNotes}" 
					id="benefitLevelNotes"
						action="#{benefitLevelNotesAttachmentBackingBean.getNotesDetailedPage}"
					onmousedown="javascript:navigatePageAction(this.id);getScroll();">
				<h:outputText value="#{node.description}" styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}" />
				</h:commandLink>
			</h:panelGroup>
