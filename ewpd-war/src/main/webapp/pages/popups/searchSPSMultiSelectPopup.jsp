...
function expandcontract(dis) {
	if(dis.keyCode=='13'){
		...
		ajaxCall1('../popups/ajaxHelperTableMulti.jsp?entityId='+entityId+'&queryName='+queryName+'&headerName='+headerName+'&benefitLvlId='+benefitLvlId+'&temp='+Math.random(),searchField,divObj,'popupForm:componentDataTable',errorMsgDiv,attrObj,'','popupForm:checkId');
		return false;
	}
}
...
function getCheckedItems(id, attrCount){
	loadAvluesToArray();
	var tableObject;
	var selectedValues = '';
	var cnt = 0;
	if (document.getElementById(id))
	{
		...
		for (var i=0;i<tableObject.rows.length;i++)
		{
			...
			popUnckeckedValue(currentCell.children[0].value);
			if (checkboxObject && checkboxObject.checked) {
				...
				selectedValues += firstCell + '~' + secondCell;	
				cnt++;
			}
		}	
	}
	...
	for(var j=0;j<checkValueArray.length;j++){
		if(cnt > 0)
			selectedValues += '~';
		var firstCell = checkValueArray[j].split('~')[0];
		firstCell = firstCell.replace('~','/w0p8d');
		var secondCell = checkValueArray[j].split('~')[1];
			while(secondCell.indexOf('~') >= 0){
				secondCell = secondCell.replace('~','/w0p8d');
			}
		selectedValues += firstCell + '~' + secondCell;	
