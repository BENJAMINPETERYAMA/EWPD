...
alert('dhdhd-->'+id);
			...
			alert('node-->'+node);
		   message='The information cannot be modified as the information is flagged for transfer to an external system.Do you want to override?';
...
*/
    //alert('id---->'+id);
	//var status=checkForUnsavedData();
	//alert('status---->'+status);
	sleep(100);
	var node = document.getElementById(id);	
...
/*****Function to check unsaved data - Start*****/

function allElement(val){

  var typeofobj = new Array("text", "select-one", "checkbox", "radio", "textarea");
  var str ="";

  for(i=0; i<document.all.length ;i++){

           for(j=0;j<typeofobj.length;j++)

           if(document.all[i].type == typeofobj[j]){

                  if(document.all[i].type =="select-one"){

                                      str = str + (document.all[i].id + document.all[i].options[(document.all[i].selectedIndex)].value);

                  }else if(document.all[i].type =="checkbox" || document.all[i].type =="radio") {

                            str = str + document.all[i].id + document.all[i].checked

                   }else{  
                   
                            str =str + (document.all[i].id +document.all[i].value);
  ...
  if(val =="init"){
           document.all.datainital.value =str;
  }

  else{
           document.all.datafinal.value =str;
...
function checkForUnsavedData(){
      alert('checkForUnsaved');
      

      allElement("final");

      if(!(document.all.datainital.value == document.all.datafinal.value)){
 
      var message = 'You have unsaved mandate information.' + '\nWant to leave this mandate?'
...
/*****Function to check unsaved data - End*****/
function tree(tm0,tm1){
	...
	this.tmB=function(tmC){var tmD=this.tm5[tmC];
	tmD.tmE(tmD.tmF)
};
...
function tmT(){
	...
	return '<table cellpadding="0" cellspacing="0" border="0" ><tr><td nowrap><font family="verdana" size="1" color="#000"> '						
	+(this.tm7?tme.join('')
	+(this.tmK.length?'<a href="javascript: trees['
	+this.tm4.tmC
	+'].tmB('
	+this.tmC
	+')" onmouseover="trees['
...
var trees=[];
tmY=document.all?function(tmh){return document.all[tmh]}:function(tmh){
