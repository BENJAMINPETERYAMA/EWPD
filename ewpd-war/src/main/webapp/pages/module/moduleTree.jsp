...
<f:subview id="tree2">
	...
	<h:inputHidden id="scrollTop" value="#{moduleTreeBackingBean.scrollTop}" />
	<aa:zoneJSF id="treeZone">
		<jcaa:ajaxTree id="serverTree" value="#{moduleTreeBackingBean.treeDataModel}"
		...
		<f:facet name="root">
			<h:panelGroup>
				<h:commandLink styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}"
					actionListener="#{moduleTreeBackingBean.processActionForModuleMainPage}" 
						action="#{moduleBackingBean.getModuleMainPage}" id="moduleRoot"
		...
		<f:facet name="Tasks">
			<h:panelGroup>
				<h:commandLink styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}"
					 actionListener="#{moduleTreeBackingBean.processActionForTask}" 
						action="#{moduleBackingBean.getTaskDetailPage}" id="moduleTasks"
							onmousedown="javascript:navigatePageAction(this.id);getScroll();">
				<h:outputText value="#{node.description}" styleClass="#{t.nodeSelected ? 'treeNodeLinkSelected':'treeNodeLink'}" />
				</h:commandLink>
			</h:panelGroup>
