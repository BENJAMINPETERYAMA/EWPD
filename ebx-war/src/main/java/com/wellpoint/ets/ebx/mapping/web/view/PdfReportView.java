...
/**
 * method to display the EWPD contract values in contract BX report - PDF view
 * @param contract
 * @return
 */
public static ArrayList getEwpdBeanCollectionData(ContractVO contract){
                                ...
                                if (mappingsIterator != null) {
                                    while (mappingsIterator.hasNext()) {
                                        ...
                                        if (mappingFromMap != null) {
                                        	ContractDataObjectVO contractEwpdDataObject = null;
		...
		if (accumListIterator != null) {
			while (accumListIterator.hasNext()) {
				...
				if (null != accumCode.getValue()) {
					...
					contractEwpdDataObject.setAccum1(DomainConstants.EMPTY);
				}
			}
		}
	}
...
/**
 * method to display the WPD contract values in contract BX report - PDF view
 * @param contract
 * @return
 */
public static ArrayList getWpdBeanCollectionData(List contractDataList){
	                                        ...
	                                        if(mappingFromMap != null) { 
	                                        	...
	                                        	if(mappingFromMap.isMapped()) {
	                                        		if(null != mappingFromMap.getEb03()) {
                                                    	...
                                                    	/*if(null != unAssocioatedList && unAssociatedList.size() != 0) {
                                                    		unAssnEb03CSV = StringUtils.join(unAssociatedList.toArray(), ",");
                                                    	}*/
                                                    	if(null != associatedEB03AssnList && associatedEB03AssnList.size() != 0) {
                                                    		for(EB03Association eb03AssnValue : associatedEB03AssnList)
...
private static List<EB03Association> getAssociatedEB03CodesForEwpd(Mapping ruleMapping,Mapping msgMapping,List<String> associatedList,
	...
	/**
	 * method to display the headings in contract BX report-PDF view based on the system.
	 * @param contractDataList
	 * @return
	 * @throws JRException
	 */
	public Map getContractReportData(List contractDataList) throws JRException {
		Map parameters = new HashMap();
		if (contractDataList != null) {
		for (int i = 0; i < contractDataList.size(); i++) {
			ContractVO contract = (ContractVO) contractDataList.get(i);
			parameters.put(JRPdfExporterParameter.PDF_JAVASCRIPT, "this.zoom = 100;");
			//Headers
			parameters.put("contractIdHeader", "Contract ID");
			parameters.put("dateSegmentHeader", "Date Segment");
			parameters.put("pvaHeader", "PVA");
			parameters.put("formatHeader", "Format");
			parameters.put("codedValueHeader", "Coded Value");
			parameters.put("eb01Header", "EB01");
			parameters.put("eb02Header", "EB02");
			parameters.put("eb03Header", "EB03");
			parameters.put("eb06Header", "EB06");
			parameters.put("eb09Header", "EB09");
			parameters.put("iii02Header", "III02");
			parameters.put("messageHeader", "MESSAGE");
			parameters.put("msgReqHeader", "MSG REQ IND");
			parameters.put("medssaeTypeHeader", "MESSAGE TYPE");
			parameters.put("accum1Header", "ACCUM 1");
			parameters.put("accum2Header", "ACCUM 2");
			parameters.put("accum3Header", "ACCUM 3");
			parameters.put("accum4Header", "ACCUM 4");
			parameters.put("accum5Header", "ACCUM 5");
			parameters.put("accum6Header", "ACCUM 6");
			parameters.put("accum7Header", "ACCUM 7");
			parameters.put("accum8Header", "ACCUM 8 ");
			parameters.put("accum9Header", "ACCUM 9");
			parameters.put("accum10Header", "ACCUM 10");
			parameters.put("sensitiveIndHeader", "SENSITIVE IND");
			parameters.put("hsd01Header", "HSD01");
			parameters.put("hsd02Header", "HSD02");
			parameters.put("hsd03Header", "HSD03");
			parameters.put("hsd04Header", "HSD04");
			parameters.put("hsd05Header", "HSD05");
			parameters.put("hsd06Header", "HSD06");
			parameters.put("hsd07Header", "HSD07");
			parameters.put("hsd08Header", "HSD08");
			parameters.put("umRuleHeader", "UM Rule");
			parameters.put("notFinalizedHeader", "Finalized");
			parameters.put("notApplicableHeader", "Not Applicable");
			if (DomainConstants.SYSTEM_LG.equalsIgnoreCase(contract.getSystem())) {
				JRBeanCollectionDataSource wpdSource = new JRBeanCollectionDataSource(PdfReportView.getWpdBeanCollectionData(contractDataList));
				parameters.put("contractDataSource", wpdSource);
				parameters.put("majorHeadingHeader", "Major Heading");
				parameters.put("minorHeadingHeader", "Minor heading");
				parameters.put("variableDescHeader", "Variable Desc");
				parameters.put("variableHeader", "Variable");
				parameters.put("startAgeHeader", "Start Age");
				parameters.put("endAgeHeader", "End Age");
				...
				JRBeanCollectionDataSource ewpdSource = new JRBeanCollectionDataSource(PdfReportView.getEwpdBeanCollectionData(contract));
				parameters.put("contractDataSource", ewpdSource);
				parameters.put("benefitComponentHeader", "Benefit Component");
				parameters.put("benefitHeader", "Benefit");
				parameters.put("ruleIdHeader", "Header Rule");
				parameters.put("tierDescrptionHeader", "Tier Details");
				parameters.put("spsIdDescHeader", "SPS Id Description");
				parameters.put("spsIdHeader", "SPS Id");
				//contract and date
				parameters.put("contractId", contract.getContractId());
				parameters.put("effectiveDate", contract.getEffectiveDate());
				parameters.put("versionHeader", "Version");
				parameters.put("version", Integer.toString(contract.getVersion()));
				}
			}
		}
		return parameters;
