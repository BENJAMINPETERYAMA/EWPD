...
var eb03ExcludedDefaultList = new Array();
...
$(document).ready(function(){
	...
	$("#EB03Id").autocomplete({ 
		...
		source: function(request, response) {
					$.ajax({
						url: "${context}/ajaxeb03defaultautocompletelistforebx.ajax",
						dataType: "json",
						type:"POST",
...
$(document).ready(function(){  		
 ...
 /*  $(document).ready(function(){  
  ...
  // $('body').live('click','#filter_age',function()	{
...
function createEb03Exclusion(currentPageNo, page)  
{  
    var eb03Default = document.getElementById('eb03Defaultval').value;
    var eb03Val = document.getElementById('EB03Id').value;
    var eb03valdesc = document.getElementById('EB03IdLabel0').innerHTML;
    var currentPage = currentPageNo;
	var fromWhichSection = page;
    
    var tableId = document.getElementById('defaltlisttbody');
	var tableRow = tableId.getElementsByTagName('tr');
	
	for (var t = 0; t < tableRow.length; t++)
	{
    	eb03ExcludedDefaultList.push(tableRow[t].firstElementChild.textContent);
	}
    
   	if(eb03ExcludedDefaultList.indexOf(eb03Val) > -1)
   	{
		alert("Default mapping is already Available");
		return;
	}
    if(eb03Val=="" || eb03Val==undefined)
    {
	    alert("Please enter Eb03 value");
	    return;
    }
    if(eb03valdesc == "" || eb03valdesc == undefined || eb03valdesc=="Invalid Eb03 Value")
	{
	   alert("Invalid EB03 Value");
	   return;
	}
	else if(eb03Default == "" || eb03Default == undefined)
	{
	   alert("Please enter EB03 default Value");
	   return;
	}
    $.ajax({
			url: "${context}/managespidereb03defaultmapping/createEB03DefaultMapping.ajax",
			dataType: "html",
			type: "POST",
			data: "&eb03Val="+encodeURIComponent(eb03Val)+"&eb03Defaultval="+encodeURIComponent(eb03Default)+ "&currentPage=" + currentPage
								+ "&section=" + fromWhichSection,
			success: function(data, status, xhr) 
			{								
				if(data.indexOf("dummyVal") > -1){
...
function deleteRowForRuleEb03(eb03,ebtr)
{
  ...
  {
	   $.ajax({
				url: "${context}/managespidereb03defaultmapping/deleteEb03DefaultMapping.ajax",
				dataType: "html",
				type: "POST",
...
function cancel()
{
	  document.getElementById('eb03Defaultval').value = "";
   	  document.getElementById('EB03Id').value = '';
      document.getElementById('EB03IdLabel0').innerHTML = '';
	/* document.forms['categoryeb03DefaultMappingForm'].action="../ebxspiderworkflow/cancel.html";
    document.forms["categoryeb03DefaultMappingForm"].submit(); */
}

function back()
{
    document.forms['categoryeb03DefaultMappingForm'].action="../ebxspiderworkflow/back.html";
    document.forms["categoryeb03DefaultMappingForm"].submit();
}

function reloadPage(){
		document.forms['categoryeb03DefaultMappingForm'].action="${context}/managespidereb03defaultmapping/showManageEB03DefaultMappingPage.html?"+ "currentPage=" + '0'
									+ "&section=" + 'fromLanding';
		document.forms['categoryeb03DefaultMappingForm'].submit();
}
function manageEB03DefaultPage(currentPageNo, page){
	var currentPage = currentPageNo;
	var fromWhichSection = page;
	document.forms['categoryeb03DefaultMappingForm'].action="${context}/managespidereb03defaultmapping/showManageEB03DefaultMappingPage.html?"
	+ "currentPage=" + currentPage + "&section=" + fromWhichSection;
	document.forms['categoryeb03DefaultMappingForm'].submit();
}
...
<body>
	...
	<form name="categoryeb03DefaultMappingForm" method="POST">

		<%@include file="/WEB-INF/jspf/pageTop.jspf"%>

		<div class="innerContainer" style="height: 98%;min-height: 400px;" class="Pd3">
			...
			<div id="exclusionEditContainer" style="height: 120px">
				...
				<div id="exclusionSectionOne1" style="height: 100px">
					<div id="exclusionTitleBarSectionOne1">
						<div class="headerTitleExclusion">Manage EB03 - EB03 Default Mapping</div>
					</div>
					<div id="exclusionContentSectionOne1" class="exclusionContentSectionOne1">
											<table style="margin-top: 3px; width: 960px;"
							class="mt10 mL10 Pd3 shadedText" id="exclusionTableWidth1"
							...
							<TBODY>
								<tr class="createEditTable1-Listdata" style="padding-top: 0px">
									...
									<td><input type="text" class="inputbox60" id="EB03Id" name="eb03Val" class="eb03" value="${EB03Id1}" onkeypress="enterKeyPress(event);" /></td>
									<td><label id="EB03IdLabel0"  for="selectedEb03" style="font-size: 11px"></label>
									  <input type="hidden" id="eb03Desc" name="eb03Label"	/>
									</td>
                                    <td width="140px" class="headTextExclusions">EB03 Default</td> 
								    <td>
								        <select class="dropdown136" id="eb03Defaultval" name="eb03Defaultval" class="inputbox60">
								...
								<tr>
									<td align="center" colspan="5">
										<a href="#">
										   <IMG id="Locate" src="${imageDir}/create_but.gif" width="70" height="18" alt="Create" onclick="createEb03Exclusion('0','fromLanding');">
										</a>
									    <a href="#">
			...
			<!-- <div class="LocateResultTableDiv"
				...
				<table border="0" cellpadding="0" cellspacing="0"
					...
					style="width: 930px;  margin-left: 15px;">
					<THEAD>
						<tr class="UnmappedVariables locateResultsTable">
							<th width="300px" class="tableHeader">EB03</th>
							<th width="400px" class="tableHeader">EB03_Default</th>
							<th width="200px" class="tableHeader">&nbsp;&nbsp;&nbsp;Actions</th>
						</tr>
					</THEAD>

					<TBODY id="defaltlisttbody">

						<c:set var="locateEb03DefaultCount" value="0" />
						<c:if test="${! empty locateEb03Default}">
							<c:forEach items="${locateEb03Default}" var="locateEb03DefaultList" varStatus="locateEb03Default">

								<tr class="${locateEb03DefaultCount mod 2 == 0 ? 'white-bg' : 'alternate'}" id="exclusionTr${locateEb03Default.index}">
									<td width="80px">${locateEb03DefaultList.eb03}</td>
									<td width="80px">${locateEb03DefaultList.eb03Default}</td>							
									<td width="130px" id="status_${locateResultList.umRuleId}">
										<a href="#">
										   <img alt="Delete Row" title="Delete Row" id="deleteRow" src="${imageDir}/delete_button.gif" width="19" height="19" onclick='deleteRowForRuleEb03("${locateEb03DefaultList.eb03}","exclusionTr${locateEb03Default.index}");' />
										</a>
									</td>
								</tr>
								<c:set var="locateEb03DefaultCount" value="${locateEb03DefaultCount + 1}" />
							</c:forEach>
						</c:if>
						<c:if test="${empty locateEb03Default}">
							<tr>
				...
				<c:if test="${! empty locateEb03Default}">
					<div style="padding-left: 365px; padding-top: 10px;">
		
						<table>
							<tr>
								<td><c:if test="${page.totalNoOfPages != 1}">
										<a href="#"
											onclick='manageEB03DefaultPage("${page.currentPage}","First");'><img
											src="${imageDir}/First Page.gif" /></a>
										<a href="#"
											onclick='manageEB03DefaultPage("${page.currentPage}","Previous");'><img
								...
								<td><c:if test="${page.totalNoOfPages != 1}">
										<a href="#"
											onclick='manageEB03DefaultPage("${page.currentPage}","Next");'><img
											src="${imageDir}/Next.gif" /></a>
										<a href="#"
											onclick='manageEB03DefaultPage("${page.totalNoOfPages}","Last");'><img
