...
<script type="text/javascript">
	...
	function populateEb03ValuesToEB03AssnList(){
		...
		var result = createEB03AssnObject();
			if(result == true){
				...
				$.ajax({
				url: "${context}/createpage/populateEb03AssociatedValues.html",
				dataType: "json",
				type: "POST",
	...
	function createEb03AssnTable(eb03AssnList) {
		...
		headElement5.innerHTML =  "Extnd Reqd";
		...
		for(var j = 0; j < eb03AssnObjCount; j++){
			...
			if(extndMsgJsonObj !== ""){			
				...
				for(index=0; index < msgObj.length; index++) {
					...
					if(obj.eb03Val == eb03Value && (obj.eb03ExtndMessage1 != "" || obj.eb03ExtndMessage2 != "" || obj.eb03ExtndMessage3 != "" )){
			...
			if (msgRqdInd == 'Y') {
				cell8.getElementsByTagName("input").item(0).checked = true;
			} else if (msgRqdInd == 'N') {
				cell8.getElementsByTagName("input").item(0).checked = false;
			}
	...
	function sniffer() {
	   ...
	   {
	      document.getElementById('createEditContainer').style.height = "450px";	   
	   } 
	   ...
	   {
	      document.getElementById("createEditContainer").style.height= "380px";
	   }
...
// Method to submit the values in  'Create Button'
	function createMapping(){
		...
		var maxLength = validateMaxLengthMsg();
		if( pass && maxLength && (imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){
			trimTextValues();
			setDescription();
			...
			document.forms['submitCreateForm'].action="${context}/createpage/create.html"+queryString;
...
// Cancel action 
	function cancel(){
		    ...
		    if(pageFrom == "advanceSearch"){
				action = "${context}/createpage/cancelToAdvanceSearchBxResult.html";
			}else{
				action = "${context}/createpage/cancel.html";
			}
...
// To check the maxlength
	...
	function validateEb01ExtMsg(){
		if ($('#eb01ExtndMsgChkBox').is(':checked') && getTheValuesInTextBox("EB01Id","VALUE")[0] == "D"){
			...
			if(obj == ""){
				...
				if(msgObj.eb01ExtndMessage1 == "" && msgObj.eb01ExtndMessage2 == "" && msgObj.eb01ExtndMessage3 == "") {
					eb01ForExtndMsgRqd = getTheValuesInTextBox("EB01Id","VALUE")[0];
				}
			}
	...
	function checkExtndMsgRqdInd(){
		...
		if (indEb03AssnChd){
			...
			for(var i = 1; i < eb03AssnTable.getElementsByTagName("TR").length; i++){
				...
				var found = false;
				var extndMsgJsonObj = $("#eb03ExtndMsgJsonObj").val();
				//var eb03ExtndMsgSysId = $("#eb03ExtndMsgSysId"+eb03Value).val();
				if(extndMsgRqdChkd){
					if(extndMsgJsonObj !== ""){			
						var msgObj = JSON.parse(extndMsgJsonObj);
	...
	function markVariableAsNotApplicable(){
		document.forms['submitCreateForm'].action="${context}/createpage/markVariableAsNotApplicable.html";
		document.forms['submitCreateForm'].submit();
     }
	...
	function spellFinMapping(cancelled){	
...
function changeCommentsConfirm(){
	...
	if(action == 'notApplicable'){
		markVariableAsNotApplicable();
	}else{
		sensitiveBnftConfirmationDialog();
	}
...
function showExtndMessagesScreen(actionId){	
	...
	if(actionId=="eb01ExtndMsgLink"){
		if($("#eb01ExtndMsgJsonObj").val()===""){
			...
			var obj= $("#eb01ExtndMsgJsonObj").val();
			var msgObj = JSON.parse(obj)[0];
			setExtndMsgs(msgObj.eb01ExtndMessage1,msgObj.eb01ExtndMessage2,msgObj.eb01ExtndMessage3,msgObj.eb01NetworkInd);
		...
		if(extndMsgsFromUI !== ""){			
			...
			for(i=0; i< msgObj.length; i++) {
				...
				if(obj.eb03Val === eb03Val){
					setExtndMsgs(obj.eb03ExtndMessage1,obj.eb03ExtndMessage2,obj.eb03ExtndMessage3,obj.eb03NetworkInd);
					flag=false;
					break;
				}
			}	
		} if(flag && eb03Val == $("#eb03Value"+eb03Val).val()){	
			var msg1 = $("#eb03ExtndMsg1"+eb03Val).val();
			var msg2 = $("#eb03ExtndMsg2"+eb03Val).val();
			var msg3 = $("#eb03ExtndMsg3"+eb03Val).val();
			var networkInd = $("#eb03NetworkInd"+eb03Val).val();			
			setExtndMsgs(msg1,msg2,msg3,networkInd);
			flag=false;
		} else if(flag){
			setExtndMsgs("","","","");
...
function storeExtdMsgsForCopyto(){
...
function saveExtndMsgs(){
	...
	if(checkboxId=="eb01ExtndMsgLink"){			
		var eb01ExtndMsgJSON = [];
		//push the value to the json array
			eb01ExtndMsgJSON.push({"eb01NetworkInd": networkInd, "eb01ExtndMessage1": extndMessage1,"eb01ExtndMessage2":extndMessage2, "eb01ExtndMessage3":extndMessage3});
			//stringify the json array and assign the same to a hidden request parameter
		eb01ExtndMsgJSON = JSON.stringify(eb01ExtndMsgJSON);
		$("#eb01ExtndMsgJsonObj").val(eb01ExtndMsgJSON);
	}else{	
		var eb03Val = $("#EB03Label" + checkboxId.substring(8)).html().trim();
		/* var eb03ExtndMsgSysId = $("#eb03ExtndMsgSysId"+eb03Val).val();
		if(eb03ExtndMsgSysId === undefined){
			eb03ExtndMsgSysId="";
		} */
		if($("#eb03ExtndMsgJsonObj").val()===""){			
			var eb03ExtndMsgJSON = [];
			//push the value to the json array
			eb03ExtndMsgJSON.push({"eb03Val": eb03Val, "eb03NetworkInd": networkInd, "eb03ExtndMessage1": extndMessage1,"eb03ExtndMessage2":extndMessage2, "eb03ExtndMessage3":extndMessage3});
			//stringify the json array and assign the same to a hidden request parameter
			eb03ExtndMsgJSON = JSON.stringify(eb03ExtndMsgJSON);
			$("#eb03ExtndMsgJsonObj").val(eb03ExtndMsgJSON);
		} else {
			...
			arrayObj.push({"eb03Val": eb03Val, "eb03NetworkInd": networkInd, "eb03ExtndMessage1": extndMessage1,"eb03ExtndMessage2":extndMessage2, "eb03ExtndMessage3":extndMessage3});
			//stringify the json array and assign the same to a hidden request parameter
			arrayObj = JSON.stringify(arrayObj);
			$("#eb03ExtndMsgJsonObj").val(arrayObj);
		}
...
<body onload="storeinputs(); storeeb03values(); disableChangeCommentsDiv(); loadAgeTierTextBox(); displayIndvidualEB03AssnPanel(); enableDisableExtndMsgCheckBox();storeExtdMsgsForCopyto();">
	...
	<c:forEach items="${mapping.eb03.extendedMsgsForSelectedValues}" var="eb03ExtMsgsFromDB" >
		<input type="hidden" id="eb03Value${eb03ExtMsgsFromDB.value}" name="eb03Value" value="${eb03ExtMsgsFromDB.value}"/>
		<input type="hidden" id="eb03ExtndMsg1${eb03ExtMsgsFromDB.value}" name="eb03ExtndMsg1" value="${eb03ExtMsgsFromDB.extndMsg1}"/>
		<input type="hidden" id="eb03ExtndMsg2${eb03ExtMsgsFromDB.value}" name="eb03ExtndMsg2" value="${eb03ExtMsgsFromDB.extndMsg2}"/>
		<input type="hidden" id="eb03ExtndMsg3${eb03ExtMsgsFromDB.value}" name="eb03ExtndMsg3" value="${eb03ExtMsgsFromDB.extndMsg3}"/>
		<input type="hidden" id="eb03NetworkInd${eb03ExtMsgsFromDB.value}" name="eb03NetworkInd" value="${eb03ExtMsgsFromDB.networkInd}"/>
	</c:forEach>	
	<input type="hidden" id="ebo1Value" name="ebo1Value" value="${mapping.eb01.extendedMsgsForSelectedValues[0].value}"/>
	<input type="hidden" id="eb01ExtndMsg1" name="eb01ExtndMsg1" value="${mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg1}"/>
	<input type="hidden" id="eb01ExtndMsg2" name="eb01ExtndMsg2" value="${mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg2}"/>
	<input type="hidden" id="eb01ExtndMsg3" name="eb01ExtndMsg3" value="${mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg3}"/>
	<input type="hidden" id="eb01NetworkInd" name="eb01NetworkInd" value="${mapping.eb01.extendedMsgsForSelectedValues[0].networkInd}"/>
	<%@include file="/WEB-INF/jspf/pageTop.jspf"%>
        ...
        <input type="hidden" id="userComments" value="" name="userComments" />
            ...
            <!-- innerContainer Starts-->
               <div id="variableInfoDiv" class="overflowContainerVariable">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="Pd3">
                  <THEAD>
						<tr class="createEditTable1">
		                    ...
		                    <td width="220" class="tableHeader">Description</td>
		                    <td width="50" class="tableHeader">PVA</td>	
							<td width="70" class="tableHeader">Data Type</td>
							<td width="70" class="tableHeader">Category</td>
							<td width="65" class="tableHeader">System</td>
		                    <td width="155" class="tableHeader">Minor Heading</td>
		                    <td width="150" class="tableHeader">Major Heading</td>
		                    <td width="150px" class="tableHeader">WPD Accumulator</td>
				...
				<TBODY>
					...
					<tr>
					<input type="hidden" id="providerArrangement" name="providerArrangement" value="${variableWithInfoList[0].pva}"/>
					<input type="hidden" id="variableFormat" name="variableFormat" value="${variableWithInfoList[0].variableFormat}"/>
					<input type="hidden" name="variableIdHidden" id="variableIdHidden" value="${variableWithInfoList[0].variableId}"/>
					<input type="hidden" name="variableDescHidden" id="variableDescHidden" value="${variableWithInfoList[0].description}"/>
					<input type="hidden" id="token" value="${TRANSACTION_TOKEN_KEY}" name="tokenKey" />
					<input type="hidden" id="isgCatagory" name="isgCatagory" value="${variableWithInfoList[0].isgCatagory}"/>
					<input type="hidden" id="lgCatagory" name="lgCatagory" value="${variableWithInfoList[0].lgCatagory}"/>
					<input type="hidden" id="system" name="system" value="${variableWithInfoList[0].variableSystem}"/>
						<td>${variableWithInfoList[0].variableId}</td>
						<td>${variableWithInfoList[0].description}</td>
						<td>${variableWithInfoList[0].pva}</td>
						<td>${variableWithInfoList[0].dataType}</td>
						<td>						
							<c:if test="${variableWithInfoList[0].lgCatagory != null}">
									${variableWithInfoList[0].lgCatagory}
							</c:if>
							<c:if test="${variableWithInfoList[0].lgCatagory == null && variableWithInfoList[0].isgCatagory != null}">
									${variableWithInfoList[0].isgCatagory}
							</c:if>
						</td>
						<td>${variableWithInfoList[0].variableSystem}</td>						
						<td>${variableWithInfoList[0].minorHeadings}</td>
						<td>${variableWithInfoList[0].majorHeadings}</td>
						<td>${accumList[0]}</td>
						<td></td>	
					</tr>
					<c:set var="counter" value="1" />
					<input type="hidden" id="minorheading0" name="minorheading" value="${variableWithInfoList[0].minorHeadings}"/>
					<input type="hidden" id="majorheading0" name="majorheading" value="${variableWithInfoList[0].majorHeadings}"/>
					<c:set var="variableInfoDivScroll" value="false" />	
					<c:forEach items="${variableWithInfoList}" var="variableList" begin ="1">
						...
						<tr class="${rowCount mod 2 == 0 ? 'alternate' : 'white-bg'}">
							...
							<td></td>
							<td></td>
							<td>${variableList.minorHeadings}</td>
							<td>${variableList.majorHeadings}</td>		
							<td>${accumList[counter]}</td>
							<td></td>				 
						</tr>
					<c:set var="rowCount" value="${rowCount + 1}"/>
					<c:set var="counter" value="${counter + 1}"/>
					<input type="hidden" id="minorheading${rowCount}" name="minorheading" value="${variableList.minorHeadings}"/>
					<input type="hidden" id="majorheading${rowCount}" name="majorheading" value="${variableList.majorHeadings}"/>
					</c:forEach> 
	...
	<!--BXNI June Change-->
				...
				<table width="950" border="0" cellpadding="0" cellspacing="0" class="Pd3 mT5 ">
                   <tr>
                    	<td class="headText" colspan="2" align ="left" nowrap>&#160;                   		
                    		...
                    		<c:set var="ifChecked" value=""></c:set>									
							<c:if test="${(mapping.eb01.extendedMsgsForSelectedValues[0].value =='D') && (mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg1 != null || mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg2 != null || mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg3 != null )}">
                    				 <c:set var="ifChecked" value="checked"></c:set>
							</c:if>
							<input type="checkbox" ${ifChecked} name="eb01ExtndMsgChkBox" id="eb01ExtndMsgChkBox" onclick="showHideExtndMsgLink(this.id)" />&#160;Extended Message&#160;&#160;&#160;&emsp;&emsp; 
							<c:if test="${(mapping.eb01.extendedMsgsForSelectedValues[0].value =='D') && (mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg1 != null || mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg2 != null || mapping.eb01.extendedMsgsForSelectedValues[0].extndMsg3 != null )}">
	                  ...
	                  <tr class="createEditTable1-Listdata alternate">
	                    ...
	                    <td width="145px">
							...
							<input <c:if test="${mapping.procedureExcludedInd == 'Y'}">checked</c:if> 
								type=checkbox name=excludeProceduresChkBox id=excludeProceduresChkBox value="checked"/>
							<label class=headText noWrap align=left id="lblProceduresExcluded">Procedures Excluded</label>
							<a href="#"  tabIndex="-1" id="hrefProcedureExcluded"
								onclick="displayInfo('EXCLUDE PROCEDURE',event,'../ajaxhippasegmenttooltip.ajax');"
								>?</a>&#160;
						</td>
						 <td width="20px">&#160;</td>
	               		<td width="70px" >						
								...
								<tr class="${rowCount mod 2 == 0 ? 'alternate' : 'white-bg'}">
									...
									<td width="150px" align="left">
										<div>
											<c:set var="ifChecked" value=""></c:set>									
											<c:forEach items="${mapping.eb03.extendedMsgsForSelectedValues}" var="eb03ExtMsgsFromDB" >
												<c:if test="${(eb03ExtMsgsFromDB.value == eB03AssnList[count].eb03.value) && (eb03ExtMsgsFromDB.extndMsg1 != '' 
		...
		<!--HSD  Table-->		
			<div>
				<table width="950" border="0" cellpadding="0" cellspacing="0" class="Pd3 mT5 bT" id="hsdTable">
					...
					<tr class="createEditTable1-Listdata">
	                    ...
	                    what is this?</a>&nbsp;
	                    <a href="#" tabindex="-1"><img src="/wsbebx/images/icon-popup.gif" width="15" height="14" id="HSD01" onclick="popupDiv('HSD01','../ajaxpopup.ajax')"></a>
	                  	</td>
	                  	 <td width="20px">&nbsp;</td>
	                    ...
	                    what is this?</a>&nbsp;
	                    <a href="#" tabindex="-1"><img src="/wsbebx/images/icon-popup.gif" width="15" height="14" id="HSD03" onclick="popupDiv('HSD03','../ajaxpopup.ajax')"></a>
	                    </td>
	                     <td width="20px">&nbsp;</td>
	                  ...
	                  <tr class="createEditTable1-Listdata alternate">
	                    <td width="70">							
								<input type="text" name="hsd01" id="HSD01Id" class="inputbox60 ui-autocomplete-input" value="${mapping.hsd01.hippaCodeSelectedValues[0].value}" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">
						...
						<td width="20px">&nbsp;</td>
	                    <td width="70px">						
								<input type="text" name="hsd03" id="HSD03Id" class="inputbox60 ui-autocomplete-input" value="${mapping.hsd03.hippaCodeSelectedValues[0].value}" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">
					...
					<tr class="createEditTable1-Listdata">
	                    <td class="headText" colspan="2">HSD05 <a href="#" tabindex="-1" onclick="displayInfo('HSD05',event,'../ajaxhippasegmenttooltip.ajax');">what is this?</a>&nbsp;<a href="#" tabindex="-1"><img src="/wsbebx/images/icon-popup.gif" width="15" height="14" id="HSD05" onclick="popupDiv('HSD05','../ajaxpopup.ajax')"></a></td>
    					<td width="20px">&nbsp;</td>
	                    ...
	                    <td width="20px">&nbsp;</td>
	                    <td class="headText" colspan="2">HSD07 <a href="#" tabindex="-1" onclick="displayInfo('HSD07',event,'../ajaxhippasegmenttooltip.ajax');">what is this?</a>&nbsp;<a href="#" tabindex="-1"><img src="/wsbebx/images/icon-popup.gif" width="15" height="14" id="HSD07" onclick="popupDiv('HSD07','../ajaxpopup.ajax')"></a></td>
					...
					<tr class="createEditTable1-Listdata alternate">
					     <td width="70px">						
								<input type="text" name="hsd05" id="HSD05Id" class="inputbox60 ui-autocomplete-input" value="${mapping.hsd05.hippaCodeSelectedValues[0].value}" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">
						...
						<td width="20px">&nbsp;</td>
	                    <td width="70px">					
								<input type="text" name="hsd07" id="HSD07Id" class="inputbox60 ui-autocomplete-input" value="${mapping.hsd07.hippaCodeSelectedValues[0].value}" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">
					  ...
					  <tr class="createEditTable1-Listdata alternate">
	                    <td width="70px">							
								<input type="text" name="hsd08" id="HSD08Id" class="inputbox60 ui-autocomplete-input" value="${mapping.hsd08.hippaCodeSelectedValues[0].value}" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">
			...
			<!--III02 Table-->		
				...
				<!--Message and NoteType Table  -->
		    		<table width="950" border="0" cellpadding="0" cellspacing="0" class="Pd3" id="messageTable">
		    			<RapidSpellWeb:rapidSpellWebLauncher id="rapidSpellWebLauncher1"
									...
									allowMixedCase="True" finishedListener="spellFinMapping"
				...
				<!-- Ends Here -->
					 	...
					 	<tr class="alternate">
		                    ...
		                    <td width="49">&#160;</td>
		                    <td width="70px" >						
									<input type="text" name="noteType" id="NOTETYPEID" class="inputbox60" id="noteTypeCodeId" value="${mapping.noteTypeCode.hippaCodeSelectedValues[0].value}" />
	...
	<c:if test="${fn:length(mapping.utilizationMgmntRule.hippaCodeSelectedValues) > 6}">
		...
		<!-- nm1MessageSegment Section Ends -->	
		 ...
		 <div class="footer">
			...
			<table border="0" align="center" cellpadding="0" cellspacing="0" class="footerTable">
		      <tr>
		        ...
		        <td width="0" height="20"><a href="#" onClick='loadChangeCommentsDialog("create");'>Create</a></td>
...
<div id="changeCommentsDialog" style="display:none;" >
	...
	<table border="0" align="center" cellpadding="0" cellspacing="0" class="footerTable">
      <tr>       
        <td>
			<a href="#" onclick='changeCommentsConfirm();'>
...
<div id="extendedMsgScreen" style="display:none;">
	<table border="0" cellspacing="0" width="100%">	
		...
		<tr height="40px">
			...
			<td width="20px"></td>
			<td id="variableId">${variableWithInfoList[0].variableId}</td>
		</tr>				
		<tr height="80px">
...
<%@include file="/WEB-INF/jspf/messageTray.jspf"%>
	...
	<script type="text/javascript">
		...
		if(umValue != 'UM' && umValue != 'um' && umValue != 'uM'
			&& umValue != 'Um'){
