    ...
    switch(id)
	   {
	     case 'spsresId' :
	     {
		     document.getElementById('spsresIcon').style.display= 'block';
		     document.getElementById('descresIcon').style.display= 'none';
		     document.getElementById('dateresIcon').style.display= 'none';  
		     document.getElementById('statusresIcon').style.display= 'none';
		     break;
	     }
	     case 'descresId' :
	     {
		     document.getElementById('spsresIcon').style.display= 'none';
		     document.getElementById('descresIcon').style.display= 'block';
		     document.getElementById('dateresIcon').style.display= 'none';  
		     document.getElementById('statusresIcon').style.display= 'none';
		      break;
	     }
	     case 'dateresId' :
	     {
		     document.getElementById('spsresIcon').style.display= 'none';
		     document.getElementById('descresIcon').style.display= 'none';
		     document.getElementById('dateresIcon').style.display= 'block';  
		     document.getElementById('statusresIcon').style.display= 'none';
		      break;
	     }
	     case 'statusresId' :
	     {
		     ...
		     document.getElementById('descresIcon').style.display= 'none';
		     document.getElementById('dateresIcon').style.display= 'none';  
		     document.getElementById('statusresIcon').style.display= 'block';
	     }
	   }
 }	     
$(document).ready(function() { 
	     ...
	     var strLocateSPS=$("#locateSPS tbody tr td").html();
	     
	     if($.trim(strLocateSPS) != 'No Results matching your search criteria')
         {
	   		 $('#locateSPS').dataTable(
					{ 			   
							"bPaginate": false ,				    
...
<table style="width: 708px;" border="0" cellpadding="0" cellspacing="0" id="locateSPS" class="locateTable locateT shadedText">
	<THEAD>
	<tr class="UnmappedVariables locateResultsTable">
		<th id="spsresId" onclick="displaySortMark(this.id)" width="130px" class="tableHeader">
		SPS ID
		<span id="spsresIcon"  class="ui-widget-header" style="padding-left:5px;display:inline-block;background:none; border:0px; position:relative; top:-4px; left:-7px;height:10px;">
						<span class="ui-icon ui-icon-carat-1-s" style="position:right;height:10px;"/>
				</span>
				</span>
		</th>
		<th id="descresId" onclick="displaySortMark(this.id)" width="250px" class="tableHeader">
		Description
		<span id="descresIcon"  class="ui-widget-header" style="padding-left:5px;display:inline-block;background:none; border:0px; position:relative; top:-4px; left:-7px;height:10px;">
						<span class="ui-icon ui-icon-carat-1-s" style="position:right;height:10px;"/>
				</span>
				</span>
		</th>
		<th id="dateresId" onclick="displaySortMark(this.id)" width="80px" class="tableHeader">
		Date&nbsp;Created
		<span id="dateresIcon"  class="ui-widget-header" style="padding-left:5px;display:inline-block;background:none; border:0px; position:relative; top:-4px; left:-7px;height:10px;">
						<span class="ui-icon ui-icon-carat-1-s" style="position:right;height:10px;"/>
				</span>
				</span>
		</th>
		<th id="statusresId" onclick="displaySortMark(this.id)" width="130px" class="tableHeader">
		Status
		<span id="statusresIcon"  class="ui-widget-header" style="padding-left:5px;display:inline-block;background:none; border:0px; position:relative; top:-4px; left:-7px;height:10px;">
						<span class="ui-icon ui-icon-carat-1-s" style="position:right;height:10px;"/>
				</span>
				</span>
		</th>
	...
	<c:if test="${! empty locateResultsList}">
		<c:forEach items="${locateResultsList}" var="locateResultList">
			<tr
				...
				<td width="250px">${locateResultList.spsId.spsDesc}</td>
				<td width="80px"><fmt:formatDate pattern="MM/dd/yyyy"
					value="${locateResultList.lastChangedTmStamp}" /></td>
				<td width="130px" id="status_${locateResultList.spsId.spsId}">${locateResultList.variableMappingStatus}</td> 
				...
				<c:if
					...
					<c:if test="${unmapped == 1}">
						<a href="#"
							onclick='openViewMappingDialogForSps("${locateResultList.spsId.spsId}");'><img
							src="${imageDir}/search_icon.gif" 
							alt="View" title="View"/></a>
					</c:if>
					<c:if test="${unmapped != 1}">
						<a href="#"
							onclick='openViewMappingInProgressDialogForSps("${locateResultList.spsId.spsId}");'><img
							src="${imageDir}/search_icon.gif" 
							alt="View"  title="View"/></a>
					</c:if>

					<c:if test="${unmapped != 1 && notapp != 1 && lock != 1}">
						<a href="#" id="locate_EditIcon_${locateResultList.spsId.spsId}"
							onclick='editMappingForSps("${locateResultList.spsId.spsId}");'><img
					...
					<input type="hidden" name="lastUpdatedUserId" id="lastUpdatedUserId" value="${locateResultList.user.lastUpdatedUserName}" />
						<c:if test="${userUIPermissions.authorizedToUnlock}">
							<a href="#" id="locate_UnlockIcon_${locateResultList.spsId.spsId}"
								onclick='unlockMappingFromLocate("","${locateResultList.spsId.spsId}","unlockSpsFromLocate", "${locateResultList.lock.lockUserId}");'><img
					...
					<c:if test="${unmapped == 1}">
						<a href="#"
							onclick='openCreateFromUnmappedSectionForSps("${locateResultList.spsId.spsId}");'><img
				...
				<a href="#"
								onclick='openUserCommentsNotApplicableDialogForSps("${locateResultList.spsId.spsId}");'>
					...
					<c:if
						...
						<a href="#" id="locate_SendToTestIcon_${locateResultList.spsId.spsId}"
							onclick='openUserCommentsDialogForSps("${locateResultList.spsId.spsId}","Send2TestSpsFromLocate");')><img
					...
					<c:if
						...
						<c:if test="${userUIPermissions.authorizedToapprove || objTransfrd==1}">
							<a href="#" id="locate_approveIcon_${locateResultList.spsId.spsId}"
								onclick='openUserCommentsDialogForSps("${locateResultList.spsId.spsId}","ApproveSpsFromLocate");')><img
