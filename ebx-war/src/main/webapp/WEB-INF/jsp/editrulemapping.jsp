...
<script type="text/javascript">
	...
	function imposeMaxLength(elementId, MaxLen, element){
	 ...
	 function sendToTest(ruleId, ruleDesc){
		...
		var result = createEB03AssnObject();
			...
			if((imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){		
				...
				document.forms['editForm'].action="${context}/createoreditrulemapping/sendToTest.html"+queryString;
	            document.forms["editForm"].submit();
           } 
          
      }
	function approve(ruleId, ruleDesc){
		...
		var result = createEB03AssnObject();
			...
			if((imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){	
				...
				document.forms['editForm'].action="${context}/createoreditrulemapping/approve.html"+queryString;
	            document.forms["editForm"].submit();
			}
      }
	function notApplicable(ruleId, ruleDesc){
			$("#selectedruleId").val(ruleId);
			$("#ruleDesc").val(ruleDesc);	
			if((imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){		
				trimTextValues();
				...
				document.forms['editForm'].action="${context}/createoreditrulemapping/notApplicable.html";
	            document.forms["editForm"].submit();
	         }   			
      }
	function discardChanges(ruleId, ruleDesc){
			$("#selectedruleId").val(ruleId);
			$("#ruleDesc").val(ruleDesc);
			if((imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){
				var action = "${context}/createoreditrulemapping/discardChanges.html";
	...
	function cancel(ruleId){
			...
			if(pageFrom == "advanceSearchEbx"){
				action = "${context}/createoreditrulemapping/cancelToAdvanceSearchPage.html?selectedruleId="+ruleId;
			}else{
				action = "${context}/createoreditrulemapping/cancel.html?selectedruleId="+ruleId;
			}
			openConfirmationDialog(action);
	}
	function save(ruleId, ruleDesc){	
		...
		var result = createEB03AssnObject();
			...
			if((imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){	
				...
				document.forms['editForm'].action="${context}/createoreditrulemapping/saveMapping.html"+queryString;
		        document.forms["editForm"].submit();
			}
      }	
	function done(ruleId, ruleDesc){
		...
		var result = createEB03AssnObject();
			...
			if((imposeMaxLength('changeCommentsId', '250', 'Change Comments'))){	
				...
				document.forms['editForm'].action="${context}/createoreditrulemapping/done.html"+queryString;
		        document.forms["editForm"].submit();
			}
      }
	function openViewHistoryDialog(ruleId) {
		$.ajax({
			url: "${context}/createoreditrulemapping/viewHistory.ajax",
			dataType: "html",
			type: "POST",
			data: "ruleId="+ruleId,
    ...
    function copyToDialog(){	
				...
				$("#copyToRuleDialog").dialog({
					 		height:'auto',
							width:'450px',
							title: 'Copy to RuleId',
	                        show:'slide',
	                        modal: true
				});
				$("#copyToRuleDialog").dialog('open');			
	}
...
function copyTo(){
		...
		if(ruleIdForCopyTo == null || ruleIdForCopyTo == ""){ 
			...
			$.ajax({
					...
					dataType: "json",
					type: "POST",			
					data: "ruleIdForCopyTo="+ruleIdForCopyTo+"&oldRuleID="+oldRuleID+"&pageFrom="+pageFrom,
					success: function(data) {
					cache: false,
...
function copyToCreateRuleId(data, ruleIdForCopyTo, oldRuleID){
		...
		if(typeof(errorMessages) != 'undefined' && errorMessages.length >0) {
			...
			trimTextValues();
			setDescription();
			document.forms['editForm'].action="${context}/createoreditrulemapping/copyToMapping.html";
	        document.forms["editForm"].submit();		   
		}
...
function printPage() {
	...
	var param="?selectedruleId="+selectedruleId+"&eb03String="+eb03String+"&eb03DescString="+eb03DescString+"&unruleString="+encodeURIComponent(unruleString)+"&umruleDescString="+encodeURIComponent(umruleDescString)+"&individualMappingString="+encodeURIComponent(individualMappingString);
...
function sensitiveBnftConfirmationDialog(ruleId, ruleDesc,action){
   var eb03Val = getTheValuesInTextBox("EB03Id","VALUE");
   isEb03Modified();
   var sensitiveBenefitList = new Array();
   var flag = 0;
   ...
   //checks if the senstitive benefit indicator is checked
	...
	if (($('#sensitiveBenefitChkBox').is(':checked'))){
		...
		//if forcefully the indicator is removed, the conformation dialog warningMsgForcefullyChecked box is called
		  if(flag != 1){
		  confirmationDialogSensitiveBenefit(ruleId, ruleDesc,warningMsgForcefullyChecked,action);
		  }
		  else{
		  if(action =='save')
		  save(ruleId, ruleDesc);
		  else if(action =='done')
		  done(ruleId,ruleDesc);
		  else if(action =='sendToTest')
		  sendToTest(ruleId,ruleDesc);
		  else if(action =='approve')
		  approve(ruleId,ruleDesc);
		  }
	}
		...
		for(var j=0;j<sensitiveBenefitList.length;j++){
		 ...
		 //if forcefully the indicator is checked, the conformation dialog box with warningMsgForSave2 is called
		  if(flag == 1){
		  confirmationDialogSensitiveBenefit(ruleId, ruleDesc,warningMsgForcefullyUnchecked,action);
		  }
		  else{
		  if(action =='save')
		  save(ruleId, ruleDesc);
...
function confirmationDialogSensitiveBenefit(ruleId, ruleDesc,warningMsg,action){
	...
	function createNewTable(eb03AssnList){
			...
			var rowCount = table.rows.length; 
			 ...
			 var count = 0;
			 if(null != eb03AssnList[0] && null != eb03AssnList[0].eb03 && eb03AssnList[0].eb03.value != ""){
				 ...
				 var headCell3 = row.insertCell(2); 
				 var headCell4 = row.insertCell(3); 
			 
			 
			 if(null != eb03AssnList[1] && null != eb03AssnList[1].eb03 && eb03AssnList[1].eb03.value != ""){
			 
				 var headCell5 = row.insertCell(4); 
				 ...
				 var headCell6 = row.insertCell(5); 
...
<body onload="storeinputs(); storeeb03values();" onload="ruleSensitiveBenefitCheck(); displayIndividualMappingPanel(); " >
	...
	<input type="hidden" id="createdDate" name="createdDate" value="${mapping.createdDate}" /> 
	  ...
	  <div id="createEditContainer" style="margin-top:10px;">	<!--createEditContainer Starts-->			
			...
			<c:if test="${fn:length(mapping.eb03.hippaCodeSelectedValues) > 6}">
					...
					<table width="950px">					
					 <tr class="createEditTable1-Listdata alternate">
					  	 <td class="headText" nowrap style="width:315px;">
						...
						<td class=headText nowrap align=left>&nbsp;<INPUT value="checked" <c:if test="${mapping.procedureExcludedInd == 'Y'}">checked</c:if>
						 	id=excludeProceduresChkBox type=checkbox name=excludeProceduresChkBox>&#160;Procedures Excluded From Pre-authorization 
	...
	<div class="footer">
			...
			<table border="0" align="center" cellpadding="0" cellspacing="0" class="footerTable">
		      <tr>
		        ...
		        <td width="0" height="20"><a href="#" onclick ='openViewHistoryDialog("${mapping.rule.headerRuleId}");'>View history</a></td>
		        <td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
		        <td width="0" height="20"><a href="#" onclick='sensitiveBnftConfirmationDialog("${mapping.rule.headerRuleId}", "${mapping.rule.ruleDesc}","save");'>Save</a></td>
		        <td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
		        <td width="0" height="20"><a href="#" onclick='sensitiveBnftConfirmationDialog("${mapping.rule.headerRuleId}", "${mapping.rule.ruleDesc}","done");'>Done</a></td>
		        <td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
		        <td width="0" height="20"><a href="#" onclick ='cancel("${mapping.rule.headerRuleId}");'>Cancel</a></td>
		       
				
				<c:if test="${mapping.variableMappingStatus !='NOT_APPLICABLE'}" > 	
						<c:if test="${userUIPermissions.authorizedToMarkAsNotApplicable}">				
						<td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
						<td width="0" height="20"><a href="#" onclick='notApplicable("${mapping.rule.headerRuleId}", "${mapping.rule.ruleDesc}");'>Not Applicable</a></td>
						</c:if>
				</c:if>
				...
				<c:if test="${mapping.variableMappingStatus != 'SCHEDULED_TO_PRODUCTION' &&
						 ...
						 <td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
						<td width="0" height="20"><a href="#" onclick='sensitiveBnftConfirmationDialog("${mapping.rule.headerRuleId}", "${mapping.rule.ruleDesc}","sendToTest");'>Send to Test</a></td>
				</c:if>
				...
				<c:if test="${mapping.variableMappingStatus != 'SCHEDULED_TO_PRODUCTION' &&
						...
						<td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
						<td width="0" height="20"><a href="#" onclick='sensitiveBnftConfirmationDialog("${mapping.rule.headerRuleId}", "${mapping.rule.ruleDesc}","approve");'>Approve</a></td>				
						</c:if>
				</c:if>
				
				<c:if test="${discardChange != 1}">
					<td width="9" height="0"><img src="${imageDir}/seperator.gif" width="9" height="20"/></td>
			        <td width="0" height="20"><a href="#" onclick='discardChanges("${mapping.rule.headerRuleId}", "${mapping.rule.ruleDesc}");' class="dicard">Discard Changes</a></td>				
	...
	<!-- wrapper Ends-->
<div id="copyToRuleDialog" style="display:none;">
