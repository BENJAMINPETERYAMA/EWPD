...
<script type="text/javascript">
  ...
  $(document).ready(function(){
		$('#spsIdForCopyTo').blur(function() {
		   if( $(this).val()== null || $.trim($(this).val()).length < 1 ) {
		   	$("#createIdLabel1").text('');
		   }
			$('#spsIdForCopyTo').val($('#spsIdForCopyTo').val().toUpperCase());	
		});
	$("#spsIdForCopyTo").autocomplete({ 
		
		select: function(event, ui) { $("#createIdLabel1").text(ui.item.id).removeClass('invalid_hippacode_value'); },
		source: function(request, response) {
					$.ajax({
						url: "${context}/ajaxautocomplete.ajax",
						dataType: "json",
		...
		change: function(event, ui) {
			var text = $(this).val();
			if(!ui.item){
				displayLabelForSelectedItem(text,list,"createIdLabel1",inValidSPSID);
			}
		}
...
<table border="0" cellpadding="0" cellspacing="0"
	class="mt10 mL10 Pd3 shadedText">
		<tr>
		<td height="26" width="150px">SPS ID <SPAN class="star">*</SPAN></td>
		<td height="26" width="150px">
			<INPUT type="text" name="spsIdForCopyTo" id="spsIdForCopyTo" class="input" />
			<INPUT type="hidden" name="oldSPSID" id="oldSPSID" value="${mapping.spsId.spsId}" />
		</td>
		<td height="26" width="661">
			<LABEL id="createIdLabel1" for="spsIdForCopyTo" style="font-size:11px"></LABEL>
	...
	<tr>
		...
		<td height="26" width="150px">
			...
			<INPUT type="hidden" name="oldRuleID" id="oldRuleID" value="${mapping.rule.headerRuleId}" />
		</td>
		<td height="26" width="661">
